...
int audio_open(const char* name, double sample_rate, unsigned long frames_per_buffer)
{
    PaError pa_rc;
...
{
    PaError pa_rc;
    if (audio_context.instream == NULL)
    {
        memset(buffer, 0, sizeof(float) * num_samples);
        printf("Error reading audio stream: invalid stream (not opened)\n");
        return (int)paBadStreamPtr;
    }
    pa_rc = Pa_ReadStream(audio_context.instream, (void*)buffer, num_samples);
    if (pa_rc != paNoError && pa_rc != paInputOverflowed)
    {
        // On failure, zero the buffer to avoid feeding garbage into the decoder
        memset(buffer, 0, sizeof(float) * num_samples);
        printf("Error reading audio stream: %s (%d)\n", Pa_GetErrorText(pa_rc), pa_rc);
        return (int)pa_rc;
    }
    return (int)pa_rc;
}
